processThisCase <- function( case, compArgs_file, filename ) {
  ...
  if ( topconnect::currentProcessedLevel( compArgs_caseSpecific, case, 0 ) ) {
    ...
    dib <- topconnect::databaseInsertBuffer(compArgs_caseSpecific$get("dbname"),compArgs_caseSpecific$get("P"), fields, 500, updates=c('clusterid'), dbuser='root', host='localhost', password=compArgs_file$get('password') );
    ...
    dub <- topconnect::databaseUpdateBuffer(compArgs_caseSpecific$get("dbname"),compArgs_caseSpecific$get('P'), 5000, static_fields, static_values, 'time', 'clusterid', password=compArgs_file$get('password') );
